...
function toggleCombineCells(group) {
  ...
  if (turningOn) {
    // Merge everything (Planned Hours, Actual Hours, ETC) onto the first
    // row, and fold % Complete into a weighted average so Earned Hours
    // stays accurate. The second row is zeroed out and hidden - all of
    // CPI / EAC Method 1/2/3 / Variance now compute on the combined total
    // since they all derive from this single row's numbers.
