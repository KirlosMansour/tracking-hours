...
let GROUPS = [];            // fetched once from /api/groups
let STATE = null;
...
function loadState() {
  ...
  if (raw) {
    try {
      ...
      // Back-fill combinedAC for data saved before this feature existed
      STATE.projects.forEach(p => { if (!p.combinedAC) p.combinedAC = {}; });
      return;
    } catch (e) {
...
function makeProject(name) {
  return {
    ...
    combinedAC: {}   // { "30% Design": true, ... } -> groups where AC is entered as one merged total
  };
}
...
function renderPhasesTable(project) {
  ...
  // "Combine AC" toggle (only applies to the standard 2-row Drawings +
  ...
  project.phases.forEach((row, idx) => {
    ...
    const combined = combinable && !!project.combinedAC[row.group];
    ...
    if (isFirstOfGroup) {
      ...
      if (combinable) {
        groupCell += `<button class="combine-toggle" data-group="${escapeHtml(row.group)}">
          ${combined ? "↔ Split AC" : "⇄ Combine AC"}
        </button>`;
      }
    }

    // ---- Actual Hours (AC) cell - merged into one cell across the two
    // rows of a group when "combine" is active, otherwise a normal
    // per-row input. ----
    let acCell;
    if (combinable && combined) {
      if (isFirstOfGroup) {
        acCell = `<td rowspan="2">
          <input type="number" min="0" step="1" class="cell-input"
                 data-field="ac-combined" data-group="${escapeHtml(row.group)}"
                 value="${row.ac ?? 0}">
          <div class="combined-note">combined total</div>
        </td>`;
      } else {
        acCell = ""; // covered by the rowspan from the first row
      }
    } else {
      acCell = `<td><input type="number" min="0" step="1" class="cell-input" data-field="ac" data-idx="${idx}" value="${row.ac ?? 0}"></td>`;
    }
  ...
  body.querySelectorAll(".combine-toggle").forEach(btn => {
    btn.addEventListener("click", () => toggleCombineAC(btn.dataset.group));
  });
}
...
function toggleCombineAC(group) {
  const project = getCurrentProject();
  const idxs = getGroupRowIndices(project, group);
  if (idxs.length !== 2) return;

  const turningOn = !project.combinedAC[group];
  project.combinedAC[group] = turningOn;

  if (turningOn) {
    // Merge: total = whatever was in both rows, all parked on the first
    // row; second row's AC zeroes out since it's now represented by the
    // single combined cell.
    const total = (Number(project.phases[idxs[0]].ac) || 0) + (Number(project.phases[idxs[1]].ac) || 0);
    project.phases[idxs[0]].ac = total;
    project.phases[idxs[1]].ac = 0;
    toast(`${group}: Actual Hours combined into one cell`);
  } else {
    toast(`${group}: Actual Hours split back into separate rows`);
  }
...
function onCellBlur(e) {
  ...
  if (field === "ac-combined") {
    const group = e.target.dataset.group;
    const idxs = getGroupRowIndices(project, group);
    const val = Math.max(0, Number(e.target.value) || 0);
    project.phases[idxs[0]].ac = val;
    project.phases[idxs[1]].ac = 0;
    e.target.value = val;
    saveState();
    updateRowCalculations(idxs[0]);
    updateRowCalculations(idxs[1]);
    renderSummaryCards(project);
    renderStageStatus(project);
    return;
  }
...
function onCellEdit(e) {
  ...
  if (field === "ac-combined") {
    ...
    project.phases[idxs[0]].ac = rawVal === "" ? 0 : Number(rawVal);
    project.phases[idxs[1]].ac = 0;
    saveState();
    updateRowCalculations(idxs[0]);
    updateRowCalculations(idxs[1]);
...
function updateRowCalculations(idx) {
  ...
  const tr = document.querySelectorAll("#phasesBody tr")[idx];
  if (!tr) return;

  // Looked up by data-role (not fixed column index) since the AC column
  // is sometimes omitted from a row's <td> list when it's covered by a
  // rowspan from the combined-AC cell above it - indices would shift.
