...
function updateRowCalculations(idx) {
  ...
  // td order: 0 group, 1 task, 2 bac(input), 3 ac(input), 4 pct(input),
  // 5 etc(input), 6 ev, 7 cpi, 8 eac1, 9 eac2, 10 eac3, 11 variance
  cells[6].innerHTML = fmt0(c.ev);
  cells[7].innerHTML = c.cpi !== null ? fmt2(c.cpi) : '<span class="muted">N/A</span>';
  cells[8].innerHTML = c.eac1 !== null ? fmt0(c.eac1) : '<span class="muted">N/A</span>';
  cells[9].innerHTML = fmt0(c.eac2);
  cells[10].innerHTML = c.eac3 !== null ? fmt0(c.eac3) : '<span class="muted">-</span>';
  cells[11].innerHTML = `<span class="eac-main badge ${cls}">${fmtSigned(c.variance)}</span>`;
}
